			...
			<div className="mt-8 relative bg-white rounded-lg shadow w-full z-[1]">
				...
				<div className="overflow-x-auto">
					<table className="min-w-full divide-y divide-gray-200">
						...
						</thead><tbody className="bg-white divide-y divide-gray-200 font-bold">{leaderboard.map((entry, index) => {
  let overlay = "";
  if (actualPositions[index] === 1) overlay = "bg-yellow-300/10"; // Ouro
  else if (actualPositions[index] === 2) overlay = "bg-gray-300/10"; // Prata
  else if (actualPositions[index] === 3) overlay = "bg-amber-700/10"; // Bronze
  return (
    <tr key={entry.profile.id} className={`hover:bg-gray-50 relative ${overlay}`}>
      <td className="w-12 pl-2 py-4 whitespace-nowrap text-xl font-bold text-gray-500 text-center">{actualPositions[index]}º</td>
      <td className="px-6 pl-2 py-4 whitespace-nowrap">
        <div className="flex items-center">
          <div className="flex-shrink-0 h-10 w-10 relative">{entry.profile.avatar_url ? (
            <Image
